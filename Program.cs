    ...
    class Program
    {
        ...
        */
        ...
        public static int BinaryToDecimal(int binary)
        {
            try
            {
                // Write your code here and you can modify the return value according to the requirements
                int decimalNumber = 0;
                int baseValue = 1; // Initialize base value to 2^0 = 1

                // Convert binary to decimal by processing each digit
                while (binary != 0)
                {
                    int lastDigit = binary % 10; // Extract the last digit
                    decimalNumber += lastDigit * baseValue; // Update decimal number
                    baseValue *= 2; // Update base value (2^1, 2^2, ...)
        ...
        */
        ...
        public static int LargestPerimeter(int[] nums)
        {
            ...
            // Step 2: Iterate through the array to find the largest perimeter
                for (int i = 0; i < nums.Length - 2; i++)
                {
                // Check if current triplet forms a valid triangle
        ...
        */
        ...
        public static string RemoveOccurrences(string s, string part)
        {
            try
            {
                // Loop until no occurrences of part are found
                while (s.Contains(part))
                {
                    // Find the index of the leftmost occurrence of part
                    int index = s.IndexOf(part);
                    // Remove the part from s starting from the leftmost occurrence
